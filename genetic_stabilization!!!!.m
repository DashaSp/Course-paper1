function genetic_stabilization()
    ...
    if success_quad
        ...
        plot_convergence(history, k_history, best_k);
...
% Параметры генетического алгоритма по умолчанию
    ...
    params.phase = 1;                 % фаза
    params.generations_phase1 = 5000;  % сколько поколений в фазе 1
    ...
    params.mutation_amplitude = 1.0;  % амплитуда мутации
    
    % Границы поиска
    params.k_bounds = [-100, 100];        % границы для k
    ...
    % Элитизм
    params.elite_count = 5;           % количество сохраняемых лучших особей
end
...
% Основная функция генетического алгоритма
    ...
    % Инициализация
    population = initialize_population(params.pop_size, params.n, params.k_bounds, params.modes);
    best_f = inf;
    ...
    % ФАЗА 1: СТАБИЛИЗАЦИЯ
    fprintf('\n--- ФАЗА 1: СТАБИЛИЗАЦИЯ РЕЖИМОВ ---\n');
    params.phase = 1;
    
    for gen = 1:min(params.generations_phase1, params.generations)
        % Передаём текущее поколение
        params.current_gen = gen;
        ...
        F1_current = compute_F1(current_best_k, params.modes);
        if F1_current < -0.05  % достигли надёжной устойчивости
            fprintf('\n Фаза 1 завершена на %d поколении, F1 = %.4f\n', gen, F1_current);
            break;
        end
    ...
    % ФАЗА 2: ОПТИМИЗАЦИЯ ВРЕМЕНИ ЗАДЕРЖКИ
    fprintf('\n--- ФАЗА 2: ОПТИМИЗАЦИЯ ВРЕМЕНИ ЗАДЕРЖКИ ---\n');
    params.phase = 2;
    
    start_gen = gen + 1;
    for gen = start_gen:params.generations
        params.current_gen = gen;
        ...
        % Вывод прогресса
        if mod(gen, 25) == 0 || gen == start_gen
            k_str = sprintf('%.4f ', current_best_k);
            fprintf('%-6d | %-12.6f | %-40s | %-10s\n', ...
                    gen, current_best_f, ['[' k_str ']'], status);
            
            % При успехе выводим дополнительную информацию
            if current_best_f < 0
                fprintf(' УСПЕХ! F = %.6f < 0\n', current_best_f);
            end
        end
...
function population = initialize_population(pop_size, n, bounds, modes)
    ...
    for i=1:length(modes)
       try
          poles = -[5 6];               % можно масштабировать
          ki = place(modes(i).A, modes(i).b, poles);
          ki = ki(:)';
          seed = [seed; ki];
       catch
       end
    ...
    if ~isempty(seed)
       base = median(seed,1);          % хороший “центральный” k
       s = min(ceil(0.4*pop_size), size(seed,1)+20);
       population(1:min(size(seed,1),s),:) = seed(1:min(size(seed,1),s),:);
       % шум вокруг base
       for j=(min(size(seed,1),s)+1):s
          population(j,:) = base + 0.5*randn(1,n);
       end
    end
...
function F = fitness_function(k, params, gen)
    ...
    tau_res = compute_tau_res(k, params.modes, params.n);
    
    if params.phase == 1
        % Фаза 1: только устойчивость (мягкий softplus)
        beta = 30;
        stab_pen = log1p(exp(beta * F1)) / beta;
        reg = 1e-4 * sum(k.^2) * (1 - 0.5 * gen/params.generations);
        F = stab_pen + reg;
        
    else
        % Фаза 2: прямое вычисление целевой функции (как в старом коде!)
        if isfinite(tau_res) && tau_res > 0
            % Тот самый ключевой член -tau0 + tau_res
            F_core = -params.tau0 + tau_res;
        else
            F_core = 1e6;  % большой штраф
        end
        
        % Мягкий штраф за потерю устойчивости (если вдруг)
        if F1 > -1e-4
            F_core = F_core + 10 * F1;
        end
...
function tau_res = compute_tau_res(k, modes, n)
% Вспомогательная функция для вычисления оценки времени задержки
    tau_res = inf;
    
    % Пытаемся оценить время задержки (только если режимы почти устойчивы)
    F1 = compute_F1(k, modes);
    if F1 < -1e-4  % эвристика: считаем оценку, если уже близко к устойчивости
        [tL, okL] = tau_bound_lyapunov(k, modes, n);
        [tD, okD] = tau_bound_diag(k, modes);
...
% МУТАЦИЯ - гауссовская с убывающей амплитудой и отражением от границ

    if nargin < 6
        max_gen = 10000;
    end
    
    % Адаптивная сигма: убывает от amplitude до 0.2*amplitude
    sigma = amplitude * (0.2 + 0.8 * (1 - gen / max_gen));
    
    for i = 1:length(child)
        if rand() < mutation_rate
            % Гауссовский шум (нормальное распределение)
            child(i) = child(i) + sigma * randn();
            
            % Отражение от границ (вместо простого ограничения)
            if child(i) < bounds(1)
                child(i) = bounds(1) + (bounds(1) - child(i));
            end
            if child(i) > bounds(2)
                child(i) = bounds(2) - (child(i) - bounds(2));
            end
        end
    end
    
    % Финальная защита от выхода за границы
...
% Создание нового поколения с элитизмом
    ...
    immigrants = round(0.07 * pop_size);
    if immigrants > 0
        b1 = params.k_bounds(1);
        b2 = params.k_bounds(2);
        new_population(end-immigrants+1:end, :) = rand(immigrants, n_vars) * (b2 - b1) + b1;
    end
end
%% ==================== ВИЗУАЛИЗАЦИЯ ====================

function plot_convergence(history, k_history, best_k)
...
function simulate_system(modes, k, tau0, n)
    ...
    semilogy(t1, norm1, 'b-', 'LineWidth', 1.5); 
    hold on;
    ...
    for i = 1:length(t_switch)
        plot([t_switch(i), t_switch(i)], yl, 'r--', 'LineWidth', 0.5, 'HandleVisibility', 'off');
    end
    ...
    fprintf('Моделирование завершено. График построен.\n');
    drawnow; % принудительное обновление графики
end
